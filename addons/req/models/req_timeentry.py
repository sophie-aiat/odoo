...
class req_timeentry(models.Model):
    ...
    at_product_ids = fields.One2many("req.products", compute="_compute_product_ids")
    product_id = fields.Many2one("req.products", string="Product", domain="[('id', 'in', at_product_ids)]")
    product_component_ids = fields.One2many("req.components", compute="_compute_component_ids")
    component_id = fields.Many2one("req.components", string="Component")
